...
javastics = normalizePath(
  file.path(path_to_JavaStics, stics_version),
  winslash = "/"
)

# Define the workspaces ---------------------------------------------------

data_dir <- SticsRFiles::download_data(
...
test_that("run_javastics works using a custom executable: exe label", {
  usms_out = run_javastics(
    javastics = javastics,
    workspace = workspace,
    stics_exe = paste0("stics_custom_", SticsRFiles:::user_os()),
    usm = usms,
    verbose = FALSE
  )
  errored = unlist(lapply(usms_out, function(x) x$error))
