...
var_name <- "lai_n"
sim_without_forcing <- SticsOnR::stics_wrapper(
  model_options = model_options,
  var = var_name,
  situation = c("bo96iN+", "bou99t1")
)
sim_with_forcing <- SticsOnR::stics_wrapper(
  param_values = param_values,
  model_options = model_options,
  var = var_name,
  situation = c("bo96iN+", "bou99t1")
...
test_that("Parameter forcing per situation works", {
  expect_gt(
    sum(
      abs(
        sim_with_forcing$sim_list[["bo96iN+"]][[var_name]] -
  ...
  expect_identical(
    sim_without_forcing$sim_list[["bou99t1"]],
    sim_with_forcing$sim_list[["bou99t1"]]
  )
})
...
test_that("Asking results for a non existing USM using situation lead to a warning", {
  expect_warning(
    ...
    regexp = "These USMs will not be simulated"
  )
})
...
test_that("Asking results for a list of variables using var works", {
  expect_equal(
    sort(var_list),
    sort(setdiff(names(res$sim_list$`lu97iN+`), c("Plant", "Date")))
  ) # tail is used to remove Date, ian, mo, jo, jul column in sim
})
...
test_that("Wrong model path lead to an error", {
  expect_error(SticsOnR::stics_wrapper_options(
    javastics = "",
    workspace = stics_inputs_path
  ))
  expect_error(SticsOnR::stics_wrapper_options(
    javastics = paste0(stics_path, "ugly_suffix"),
    workspace = stics_inputs_path
  ))
})
...
test_that("Unexisting data path lead to an error", {
  expect_error(SticsOnR::stics_wrapper_options(
    ...
    workspace = paste0(stics_inputs_path, "ugly_suffix")
  ))
})
...
stics_inputs_path <- normalizePath(
  file.path(tempdir(), "RotationTests"),
  winslash = "/",
  mustWork = FALSE
)
dir.create(stics_inputs_path, showWarnings = FALSE)
...
SticsRFiles::gen_usms_xml2txt(
  ...
  workspace = javastics_workspace_path,
  out_dir = stics_inputs_path,
  usm = c("demo_BareSoil2", "demo_Wheat1", "banana", "demo_maize3"),
  verbose = TRUE
)
...
sim_with_successive_restricted_results <- SticsOnR::stics_wrapper(
  model_options = model_options,
  situation = c("banana", "demo_maize3"),
  var = c("lai_n", "masec_n", "azomes", "resmes")
)

maize_succ_res <- file(
  file.path(stics_inputs_path, "demo_maize3", "mod_bdemo_maize3.sti"),
  "rb"
)
nb_grep_maize <- grep("rotation", readLines(maize_succ_res))
baresoil_succ_res <- file(
  file.path(stics_inputs_path, "demo_BareSoil2", "mod_bdemo_BareSoil2.sti"),
  "rb"
)
...
test_that("Test rotation", {
  ...
  expect_identical(
    sim_with_successive$sim_list$banana,
    sim_without_successive$sim_list$banana
  )
  expect_identical(
    sim_with_successive$sim_list$demo_Wheat1,
    sim_without_successive$sim_list$demo_Wheat1
  )
  expect_false(identical(
    sim_with_successive$sim_list$demo_BareSoil2,
    sim_without_successive$sim_list$demo_BareSoil2
  ))
  expect_false(identical(
    sim_with_successive$sim_list$demo_maize3,
    sim_without_successive$sim_list$demo_maize3
  ))
  expect_identical(
    names(sim_with_successive_restricted_results$sim_list),
    c("banana", "demo_maize3")
  )
  expect_identical(
    sim_with_successive_restricted_results$sim_list$demo_maize3,
    sim_with_successive$sim_list$demo_maize3
  )
...
## Generate Stics input files from JavaStics input files
stics_inputs_path <- normalizePath(
  file.path(tempdir(), "IntercropTests"),
  winslash = "/",
  mustWork = FALSE
)
dir.create(stics_inputs_path, showWarnings = FALSE)
...
sim_intercrop <- SticsOnR::stics_wrapper(
  model_options = model_options,
  situation = c("intercrop_pea_barley")
)

test_that("Test rotation", {
  expect_true(
    length(unique(sim_intercrop$sim_list$intercrop_pea_barley$Plant)) > 1
  )
  expect_true(
    length(unique(sim_intercrop$sim_list$intercrop_pea_barley$Dominance)) > 1
  )
})
